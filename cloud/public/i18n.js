(() => {
  ...
  const en = new Map(Object.entries({
    ...
    "Дополнительные действия Vault": "More Vault actions",
    "Дополнительные действия Team Vault": "More Team Vault actions",
    "Синхронизация и блокировка": "Synchronization and locking",
    "Графит": "Graphite",
    "Изумруд": "Emerald",
    "Светлая": "Light",
    "Войти": "Sign in",
    ...
    "Адрес для подтверждения": "Address to verify",
    "Новый пароль Selective Remote": "New Selective Remote password",
    "Если потребуется подтверждение адреса, мы отправим дальнейшие инструкции. Пароли по почте не отправляются и не сохраняются в браузере.": "If address verification is required, we’ll send further instructions. Passwords are never emailed or stored in the browser.",
    ...
    "Personal Vault ещё не создан для этого аккаунта": "Personal Vault has not been created for this account yet",
    "Подтвердите пароль аккаунта, чтобы создать первый зашифрованный Vault.": "Confirm your account password to create the first encrypted Vault.",
    "Подтвердить пароль и создать Vault": "Confirm password and create Vault",
    "Подтвердите пароль аккаунта, чтобы открыть личные подключения.": "Confirm your account password to open personal connections.",
    "Подтвердить пароль и открыть Vault": "Confirm password and open Vault",
    "Требуется восстановление доступа к Personal Vault": "Restore access to Personal Vault",
    "Сессия аккаунта активна, но доверенный ключ этого браузера недоступен. Подтвердите пароль аккаунта, чтобы восстановить доступ.": "Your account session is active, but this browser's trusted key is unavailable. Confirm your account password to restore access.",
    "Восстановить доступ": "Restore access",
    "Восстанавливаем Personal Vault": "Restoring Personal Vault",
    "Проверяем локальный ключ и зашифрованную Cloud-ревизию.": "Checking the local key and encrypted Cloud revision.",
    "Personal Vault временно недоступен": "Personal Vault is temporarily unavailable",
    "Сессия активна. Повторите подтверждение пароля или синхронизацию.": "Your session is active. Confirm your password or synchronize again.",
    "Повторить подтверждение пароля": "Confirm password again",
    "Сессия активна. Подтвердите пароль, чтобы создать первый зашифрованный Personal Vault.": "Your session is active. Confirm your password to create the first encrypted Personal Vault.",
    "Сессия активна, но ключ Personal Vault не восстановлен. Подтвердите пароль или откройте другую доверенную вкладку.": "Your session is active, but the Personal Vault key was not restored. Confirm your password or open another trusted tab.",
    "Сессия активна, но доверенный ключ Personal Vault недоступен. Восстановите доступ паролем аккаунта или откройте другую доверенную вкладку.": "Your session is active, but the trusted Personal Vault key is unavailable. Restore access with your account password or open another trusted tab.",
    "Сессия активна, но состояние Personal Vault не удалось проверить.": "Your session is active, but the Personal Vault state could not be checked.",
    "Пароль не подтверждён. Сессия остаётся активной; Personal Vault не изменён.": "The password was not confirmed. Your session remains active and Personal Vault was not changed.",
    "Personal Vault не изменён. Повторите подтверждение пароля позже.": "Personal Vault was not changed. Confirm your password again later.",
    ...
    "Ресурсы выбранной команды": "Selected team resources",
    "← К настройкам команды": "← Back to team settings",
    "Схема подключённых устройств": "Connected devices diagram",
  ...
  for (const [source, translation] of Object.entries({
    ...
    "Проверьте почту. Если для этого адреса требуется подтверждение, мы отправим дальнейшие инструкции.": "Check your email. If this address requires verification, we’ll send further instructions.",
    ...
    "Создайте пароль Selective Remote. Если потребуется подтверждение адреса, дальнейшие инструкции придут по электронной почте.": "Create a Selective Remote password. If address verification is required, further instructions will arrive by email.",
    ...
    "Текущий пароль неверен. Введите пароль текущего аккаунта и повторите.": "The current password is incorrect. Enter your current account password and try again.",
    "Название не совпадает. Введите точное название Team и повторите.": "The name does not match. Enter the exact Team name and try again.",
    "Team не архивирована. Проверьте полномочия Owner и повторите попытку.": "The Team was not archived. Check your Owner permissions and try again.",
    ...
    "Personal Vault открыт только для этой страницы; сохранить ключ доверенного браузера не удалось. После перезагрузки потребуется восстановить доступ.": "Personal Vault is open for this page only; the trusted-browser key could not be saved. You will need to restore access after a reload.",
    ...
    "Personal Vault разблокирован активной вкладкой только для этой страницы; после перезагрузки может потребоваться восстановление доступа.": "Personal Vault was unlocked by an active tab for this page only; restoring access may be required after a reload.",
