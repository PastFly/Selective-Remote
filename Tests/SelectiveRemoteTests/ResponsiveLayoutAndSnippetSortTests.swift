...
func hostsAndSFTPUseSpaceEfficientLayouts() throws {
    ...
    #expect(content.contains("personalHostSidebarPresentationID = UUID()"))
    #expect(content.contains("personalHostNavigatorPresentationID = UUID()"))
    #expect(content.contains("refreshHostPresentations()"))
    #expect(content.contains("showsPersonalHostSelection(on: surface)"))
    #expect(!content.contains("List(selection: Binding("))
    #expect(content.contains("personalHostNavigatorFooter"))
    #expect(content.contains("Добавить Host"))
