...
например `0.18.3` → `0.19.0`. `BUILD_NUMBER` тоже должен монотонно расти, но
...
```bash
git tag -a v0.19.0 -m "Selective Remote 0.19.0"
...
- при английском языке macOS основные экраны, Terminal и Forwarding показаны
  на английском, а системные запросы камеры и микрофона локализованы;
