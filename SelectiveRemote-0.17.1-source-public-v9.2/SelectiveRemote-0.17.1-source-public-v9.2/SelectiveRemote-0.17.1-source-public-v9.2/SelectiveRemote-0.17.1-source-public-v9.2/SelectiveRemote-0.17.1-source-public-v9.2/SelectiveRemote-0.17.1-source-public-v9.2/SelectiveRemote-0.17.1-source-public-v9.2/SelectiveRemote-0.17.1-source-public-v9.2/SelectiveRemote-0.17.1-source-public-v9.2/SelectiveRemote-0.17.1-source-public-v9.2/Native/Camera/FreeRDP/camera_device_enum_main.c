...
UINT VCAPITYPE avfoundation_freerdp_rdpecam_client_subsystem_entry(
...
static UINT ecam_load_hal_plugin(CameraPlugin* ecam, const char* name, const ADDIN_ARGV* args)
{
	...
	PFREERDP_CAMERA_HAL_ENTRY halEntry = nullptr;
	UINT error = ERROR_INTERNAL_ERROR;

#if defined(__APPLE__)
	/* The macOS HAL is linked into the same isolated channel addin. Loading a
	 * second plugin through FreeRDP's global addin loader is unnecessary and
	 * would make the portable bundle depend on another compiled install path. */
	if (strcmp(name, "avfoundation") == 0)
	{
		halEntry = avfoundation_freerdp_rdpecam_client_subsystem_entry;
	}
	else
#endif
	{
		/* Keep the loader conversion in the same union form used by FreeRDP.
		 * The const-qualified union member is read-only by design, so the
		 * built-in AVFoundation entry point is stored in a separate variable. */
		union
		{
			PVIRTUALCHANNELENTRY pvce;
			const PFREERDP_CAMERA_HAL_ENTRY entry;
		} cnv = WINPR_C_ARRAY_INIT;

		cnv.pvce =
		    freerdp_load_channel_addin_entry(RDPECAM_CHANNEL_NAME, name, nullptr, 0);
		halEntry = cnv.entry;
	}
