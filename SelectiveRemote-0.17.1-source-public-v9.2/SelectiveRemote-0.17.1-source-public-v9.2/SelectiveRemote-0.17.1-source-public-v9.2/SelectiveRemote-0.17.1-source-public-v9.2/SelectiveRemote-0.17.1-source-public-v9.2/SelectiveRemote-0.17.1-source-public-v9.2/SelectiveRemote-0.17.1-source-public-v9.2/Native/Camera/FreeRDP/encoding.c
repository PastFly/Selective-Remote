...
{
	const char* configured = getenv("SELECTIVE_RDP_CAMERA_MAX_BITRATE");
	...
	if ((errno != 0) || (end == configured) || (end && (*end != '\0')) || (parsed == 0) ||
	    (parsed > UINT32_MAX))
	{
		WLog_WARN(TAG, "Ignoring invalid camera bitrate cap: %s", configured);
		return 0;
	}
	...
	for (size_t i = 0; i < nBitrates; i++)
	{
		...
		{
			const UINT32 recommended = bitrates[i].bitrate * 1000;
			const UINT32 bitrate = (UINT32)parsed < recommended ? (UINT32)parsed : recommended;
			WLog_DBG(TAG, "Camera H.264 bitrate cap: %u kbps", bitrate / 1000);
			return bitrate;
		}
	}
...
BOOL ecam_encoder_context_init(CameraDeviceStream* stream)
{
	...
	const UINT32 maxBitrate = h264_get_max_bitrate(stream->currMediaType.Height);

	if (!freerdp_video_context_reconfigure(stream->video, stream->currMediaType.Width,
	                                       stream->currMediaType.Height, framerate, maxBitrate,
