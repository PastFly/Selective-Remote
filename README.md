...
Настройка применяется к Terminal, SFTP и Forwarding. Для HTTP CONNECT поддерживается имя пользователя прокси. Пароль proxy пока намеренно не передаётся через аргументы процесса; полноценное безопасное хранение proxy-password требует отдельного credential helper.
