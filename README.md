...
Раздел **Cloud** показывает виртуальные машины AWS, Microsoft Azure и DigitalOcean без создания отдельной терминальной системы. Работающую VM можно импортировать как обычный SSH-профиль и открыть в существующем Terminal Workspace или SFTP.

- AWS использует регион, Access Key ID, Secret Access Key и необязательный Session Token.
- Azure использует Service Principal: Tenant ID, Client ID, Client Secret и Subscription ID.
- DigitalOcean использует Personal Access Token и, при необходимости, ограничивает список указанным Project ID.
- Секреты сохраняются только в macOS Keychain. В профиле остаются лишь ссылка на облачный ресурс и несекретные метаданные.
- Повторное обновление меняет состояние, регион, сеть и теги, но не перезаписывает имя, группу, адрес, пользователя, ключи, маршрут или Snippets, изменённые пользователем.
- Выключенные VM остаются видимыми со статусом «Выключена»; кнопки SSH/SFTP для них недоступны, поэтому остановка не выглядит как ошибка подключения.
